        ...
        while answer is None:
            try:
                if self.model in {"gpt-3.5-turbo-16k", "gpt-3.5-turbo", "gpt-4", "gpt-4-1106-preview"}:
                    ...
                    r = await aclient.completions.create(model=self.model, prompt=prompt, **OPENAI_COMPLETION_OPTIONS)
            ...
            try:
                if self.model in {"gpt-3.5-turbo-16k", "gpt-3.5-turbo", "gpt-4", "gpt-4-1106-preview"}:
                    ...
                    async for r_item in r_gen:
                        ...
                        if hasattr(delta, 'content') & isinstance(delta.content, str):
                    ...
                    prompt = self._generate_prompt(message, dialog_messages, chat_mode)
                    r_gen = await aclient.completions.create(model=self.model, prompt=prompt, stream=True,
                                                             **OPENAI_COMPLETION_OPTIONS)
...
async def generate_images(prompt, n_images=4, size="1024x1024"):
    r = await aclient.images.generate(model="dall-e-3", prompt=prompt, n=n_images, size=size)
